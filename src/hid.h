 ...
 * 
 * \warning Code in the common part of PCB may *not* rely on *anything*
 * other than what's defined in this file.\n
 ...
 * timers, etc.
 *
 * So it passes/returns one of these, which is castable to a variety of
 * things.
 */
  typedef union
  {
...
/*!
 * \brief This graphics context is an opaque pointer defined by the HID.
 *
 ...
 */
  typedef struct
  {
     ...
     * coordinates to act on, and this string may be used to prompt the
     * user to select a coordinate.\n
     * If NULL, the coordinates may be 0,0 if none are known.
     */
    const char *need_coord_msg;
    /*!
     * Called when the action is triggered.\n
     * If this function returns non-zero, no further actions will be
     * invoked for this key/mouse event.
     */
...
/*!
 * \note PCB expects the gui to provide the following actions:\n
 * - PCBChanged();
 * - RouteStylesChanged()
 * - NetlistChanged() (but core should call "void NetlistChanged(int);"
 *   in netlist.c)
 * - LayersChanged()
 * - LibraryChanged()
 * - Busy()
 ...
 */
  struct hid_st
  {
     ...
     * different hid.h than we're expecting should have a different size
     * here.
     */
    int struct_size;
     ...
     * This should be suitable for command line options, multi-selection
     * menus, file names, etc.
     */
    const char *name;
     ...
     * Exactly one of these three flags must be set; setting "gui" lets
     * the expose callback optimize and coordinate itself.
     */
    char gui:1;
     ...
     * part of the File->Export menu option.\n
     ...
     * clearances.\n
     *
     * \note HIDs may set both of these, in which case polygons will be
     * drawn twice.
     */
    char poly_after:1;
     ...
     * Call with options == NULL to start the primary GUI (create a main
     * window, print, export, etc).
     */
    ...
    /*!
     * This may be called to ask the GUI to force a redraw of a given
     * area.
     */
     ...
     * If it returns false (zero), the HID does not want that layer, and
     * none of the drawing functions should be called.\n
     * If it returns true (nonzero), the items in that layer [group]
     * should be drawn using the various drawing functions.\n
     * In addition to the MAX_LAYERS copper layer groups, you may select
     * layers indicated by the macros SL_* defined above, or any others
     * with an index of -1.\n
     ...
     * (1/100 mil at the time this comment was written).\n
     ...
     * use this function to tell the HID when you're using it.\n
     * At the beginning of a layer redraw cycle (i.e. after set_layer),
     * call use_mask() to redirect output to a buffer.\n
     * Draw to the buffer (using regular HID calls) using regular and
     * "erase" colors.\n
     * Then call use_mask(HID_MASK_OFF) to flush the buffer to the HID.\n
     * If you use the "erase" color when use_mask is disabled, it simply
...
#define HID_MASK_AFTER 3
    ...
    /*!
     * Set a color.\n
     * Names can be like "red" or "#rrggbb" or special names like
     * "erase".\n
     * *Always* use the "erase" color for removing ink (like polygon
     * reliefs or thermals), as you cannot rely on knowing the
     * background color or special needs of the HID.\n
    ...
    /*!
     * Set the line style.\n
     * While calling this is cheap, calling it with different values
     * each time may be expensive, so grouping items by line style is
     * helpful.
     */
    ...
    /*!
     * Draw a line.\n
     * One of the usual drawing functions.\n
     * "draw" means to use segments of the given width, whereas "fill"
     * means to fill to a zero-width outline.
     */
    ...
    /*!
     * Draw only the outline of a pcb polygon.\n
     * One of the usual drawing functions.\n
     * "draw" means to use segments of the given width, whereas "fill"
     * means to fill to a zero-width outline.
     */
    ...
    /*!
     * Draw only the outline of a pcb pad.\n
     * One of the usual drawing functions.\n
     * "draw" means to use segments of the given width, whereas "fill"
     * means to fill to a zero-width outline.
     */
     ...
     * If you call this for the GUI, \c xval and \c yval are ignored,
     * and a dialog pops up to lead you through the calibration
     * procedure.\n
     * For the printer, if \c xval and \c yval are zero, a calibration
     * page is printed with instructions for calibrating your printer.\n
     * After calibrating, nonzero \c xval and \c yval are passed
     ...
     * on grid and pad snap.
     *
     * \note The HID is responsible for hiding, showing, redrawing,
     * etc.
     *
     * The core just tells it what coordinates it's actually using.
     *
     * \note This routine may need to know what "pcb units" are so
     * it can display them in mm or mils accordingly.
     *
     * If cursor_action is set, the cursor or screen may be adjusted so
     * that the cursor and the crosshair are at the same point on the
     * screen.
     */
...
#define HID_SC_PAN_VIEWPORT	2
    ...
    /*!
     * Causes \c func to be called at some point in the future.\n
     * Timers are only good for *one* call; if you want it to repeat,
     * add another timer during the callback for the first.\n
     * \c user_data can be anything, it's just passed to \c func.\n
     ...
     * \v user_data can be anything, it's just passed to \c func.
     */
    ...
    /*!
     * Causes \c func to be called in the mainloop prior to blocking.
     */
     ...
     * button.\n
     * If you specify alternate labels for ..., they are used instead of
     * the default OK/Cancel ones, and the return value is the index of
     * the label chosen.\n
     * You MUST pass \c NULL as the last parameter to this.
     */
    int (*confirm_dialog) (char *msg_, ...);

    /*!
     * A close confirmation dialog for unsaved pages, for example, with
     * options "Close without saving", "Cancel" and "Save".\n
...
#define HID_CLOSE_CONFIRM_OK     1
     ...
     * If \c default_string isn't \c NULL, the form is pre-filled with
     * this value.\n
...
#define HID_FILESELECT_IS_TEMPLATE 0x04
    ...
    /*!
     * \c title may be used as a dialog box title.\n
     * Ignored if \c NULL.\n
     * \n
     * \c descr is a longer help string.\n
     * Ignored if \c NULL.\n
     * \n
     * \c default_file is the default file name.\n
     * Ignored if \c NULL.\n
     * \n
     * \c default_ext is the default file extension, like ".pdf".\n
     * Ignored if NULL.\n
     * \n
     * \c history_tag may be used by the GUI to keep track of file
     * history.\n
     * Examples would be "board", "vendor", "renumber", etc.\n
     * If \c NULL, no specific history is kept.\n
     * \n
     * \c flags are the bitwise or of the \c HID_FILESELECT defines
     * above.
     */
     ...
     * If \c n_attrs is zero, \c attrs(.name) must be \c NULL terminated.\n
     * Returns non-zero if an error occurred (usually, this means the
     * user cancelled the dialog or something).\n
     * \c title is the title of the dialog box.
     * \c descr (if not NULL) can be a longer description of what the
     ...
     * selected item.\n
     * The expose callback is called twice; once to size the extents of
     * the item, and once to draw it.\n
     * To pass magic values, pass the address of a variable created for
     * this purpose.
     */
     ...
     * These APIs must be implemented (non \c NULL), but they do not
     * have to be functional.\n
     * \c request_debug_draw can return \c NULL to indicate debug
     * drawing is not permitted.\n
     ...
     * gui-> for making drawing calls.\n
     * If the return value is \c NULL, then permission has been denied,
     ...
     * May be implemented as a \c NOOP if the GUI has chosen to send the
...
/*!
 * When PCB runs in interactive mode, this is called to instantiate one
 * GUI HID which happens to be the GUI.\n
 ...
 * If item is not \c NULL, only draw the given item.\n
 ...
 * \c hid_expose_callback also temporarily set it for drawing.
 */
  extern HID *gui;

/*!
 * This is either \c NULL or points to the current HID that is being
...
/*!
 * This is either \c NULL or points to the current HID_Action that is
 * being called.\c
