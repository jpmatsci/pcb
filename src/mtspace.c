...
/*!
 * qloop takes a vector (or heap) of regions to check (checking) if they
 * don't intersect anything.
 *
 * If a region does intersect something, it is broken into
 ...
 *
 * \return true if it has exhausted the region vector/heap and never
 * found an empty area.
 */
static void
 ...
 * *this* pass (which  are appended to the hi_conflict_space_vec
 * vector).\n
